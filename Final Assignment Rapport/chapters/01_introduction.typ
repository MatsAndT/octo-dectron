...
DroneRF-datasettet @DroneRF_dataset inneholder RF-opptak fra tre kommersielle dronemodeller i ulike aktivitetstilstander. Rå RF-signaler er lange tidsserier som ikke kan brukes direkte som modellinput. Signalene må segmenteres i vinduer og representeres på en form som egner seg for maskinlæring. Prosjektet undersøker to representasjonsstrategier: sammensatte frekvensbånds-statistikker for MLP, og direkte vinduesekvenser for CNN.

== Problemstilling

#figure([_Kan maskinlæringsmodeller klassifisere dronemodus basert på RF-signaler?_])
